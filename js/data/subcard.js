export var dataDictionary = [
  {
    imageSrc: "../img/cards/attackontitan.jpg",
    animeName: "Attack On Titan",
    starRate: "6.7",
    episodeCount: "6",
    releaseDate: "jul 14,2023",
    description:
      "It is set in a world where humanity is forced to live in cities surrounded by three enormous walls that protect them from gigantic man-eating humanoids referred to as Titans; the story follows Eren Yeager, who vows to exterminate the Titans after they bring about the destruction of his hometown and the death of his ...",
    otherNames: "Attack On Titan",
    airedDate: "Oct 10, 2010",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/bleach.jpg",
    animeName: "Bleach-season final",
    starRate: "9.7",
    episodeCount: "3",
    releaseDate: "jul 30,2023",
    description:
      "Ichigo Kurosaki is a teenager from Karakura Town who can see ghosts, a talent allowing him to meet a supernatural human Rukia Kuchiki, who enters the town in search of a Hollow, a kind of monstrous lost soul who can harm both ghosts and humans.",
    otherNames: "Bleach-season final",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/Free.jpg",
    animeName: "FREE!",
    starRate: "6.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Free! revolves around members of a high school boys' swimming team. Haruka Nanase, Makoto Tachibana, Rin Matsuoka and Nagisa Hazuki are members of the same swimming club in elementary school before parting ways, when Rin moves to Australia for schooling and specialized training in swimming.",
    otherNames: "FREE!",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/myhero.jpg",
    animeName: "Myheroacademia",
    starRate: "7.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Academies across the globe train their students to learn to fight crime with their powers. Izuku Midoriya, a boy born without any powers, dreams of being able to become a super hero too, but gets bullied for his unrealistic dreams. One day, his fate changes in an unexpected meeting with the no.",
    otherNames: "MY HERO ACADEMIA",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/deathnote.jpg",
    animeName: "Death note",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "A high school student named Light Turner discovers a mysterious notebook that has the power to kill anyone whose name is written within its pages, and launches a secret crusade to rid the world of criminals.",
    otherNames: "Death note",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/chainsawman.jpg",
    animeName: "Chainsaw man",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Following a betrayal, a young man left for dead is reborn as a powerful devil-human hybrid after merging with his pet devil and is soon enlisted into an organization dedicated to hunting devils. When his father died, Denji was stuck with a huge debt and no way to pay it back.",
    otherNames: "Chainsaw man",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/classrooma.jpg",
    animeName: "classroom asassian",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description: "",
    otherNames: "classroom asassian",
    airedDate: "Oct 20, 2016",
    status: "now",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/fairytail.jpg",
    animeName: "Fairy tail ",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      ". The world of Earth-land is home to numerous guilds where wizards apply their magic for paid job requests. Natsu Dragneel, a Dragon Slayer wizard from the Fairy Tail guild, explores the Kingdom of Fiore in search of his missing adoptive father, the dragon Igneel",
    otherNames: "fairy tail",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/demonslayer.jpg",
    animeName: "Demons slayer",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Tanjiro sets out to become a demon slayer to avenge his family and cure his sister.A family is attacked by demons and only two members survive - Tanjiro and his sister Nezuko, who is turning into a demon slowly. Tanjiro sets out to become a demon slayer to avenge his family and cure his sister.",
    otherNames: "Demons slayer",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/JujutsuKaisen.jpg",
    animeName: "Jujutsu kaisen season 2",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "The story follows high school student Yuji Itadori as he joins a secret organization of Jujutsu Sorcerers in order to kill a powerful Curse named Ryomen Sukuna, of whom Yuji becomes the host.",
    otherNames: "MY HERO ACADEMIA",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/naruto.jpg",
    animeName: "Naruto ",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "It tells the story of Naruto Uzumaki, a young ninja who seeks recognition from his peers and dreams of becoming the Hokage, the leader of his village. The story is told in two parts—the first set in Naruto's pre-teen years, and the second in his teens.",
    otherNames: "Naruto",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/onepiece.jpg",
    animeName: "onepiece",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Gold Roger, the King of the Pirates, attained this and everything else the world had to offer. But 2 decades after his death, a young man named Luffy embarks on a journey across the world to discover it's greatest treasure, the One Piece, and inherit Roger's will - by becoming the King of the Pirates.",
    otherNames: "onepiece",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
];

export var dataDictionarysub = [
  {
    imageSrc: "../img/cards/Parasyte.jpg",
    animeName: "PARASYTE",
    starRate: "6.7",
    episodeCount: "6",
    releaseDate: "jul 14,2023",
    description:
      "The 16-year-old Izumi Shinichi lives with his mother and father in a quiet neighborhood in Tokyo. One night, worm-like aliens called Parasytes invade Earth, taking over the brains of human hosts by entering through their ears or noses.",
    otherNames: "PARASYTE",
    airedDate: "Oct 10, 2010",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/mobpyscho.jpg",
    animeName: "mob pyscho",
    starRate: "9.7",
    episodeCount: "3",
    releaseDate: "jul 30,2023",
    description:
      "Shigeo Kageyama is an average middle school-aged boy, nicknamed Mob. Although he looks like an inconspicuous person, he is in fact a powerful esper with immense psychic power. To keep from losing control of this power, he constantly lives a life under an emotional shackle.",
    otherNames: "mob pyscho",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/rezero.jpg",
    animeName: "re:zero",
    starRate: "6.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "The story centers on Subaru Natsuki, a hikikomori who suddenly finds himself transported to another world on his way home from the convenience store. The series was initially serialized on the website Shōsetsuka ni Narō from 2012 onwards.",
    otherNames: "re:zero",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/steinsgate.jpg",
    animeName: "steins gate",
    starRate: "7.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "The series aired for 24 episodes, from April to September 2011. It is set in 2010 and follows Rintaro Okabe, who together with his friends accidentally discovers a method of time travel through which they can send text messages to the past, thereby changing the present.",
    otherNames: "steins gate",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/deathnote.jpg",
    animeName: "Death note",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "A high school student named Light Turner discovers a mysterious notebook that has the power to kill anyone whose name is written within its pages, and launches a secret crusade to rid the world of criminals.",
    otherNames: "Death note",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/sweet-reincarnation.jpg",
    animeName: "Sweet-reincarnation",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Nine-year-old Pastry Mille Morteln is from reincarnation and encounters the magical world. He must learn how to fight, utilizing his confectionary knowledge about pastry and discovering magic along the way.",
    otherNames: "Sweet-reincarnation",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/classrooma.jpg",
    animeName: "classroom asassian",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description: "",
    otherNames: "classroom asassian",
    airedDate: "Oct 20, 2016",
    status: "now",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/fairytail.jpg",
    animeName: "Fairy tail ",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      ". The world of Earth-land is home to numerous guilds where wizards apply their magic for paid job requests. Natsu Dragneel, a Dragon Slayer wizard from the Fairy Tail guild, explores the Kingdom of Fiore in search of his missing adoptive father, the dragon Igneel",
    otherNames: "fairy tail",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/demonslayer.jpg",
    animeName: "Demons slayer",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Tanjiro sets out to become a demon slayer to avenge his family and cure his sister.A family is attacked by demons and only two members survive - Tanjiro and his sister Nezuko, who is turning into a demon slowly. Tanjiro sets out to become a demon slayer to avenge his family and cure his sister.",
    otherNames: "Demons slayer",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/sevendeadlysins.jpg",
    animeName: "seven deadly sins",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "The Seven Deadly Sins are a band of knights in the land of Britannia (ブリタニア, Buritania) who had disbanded ten years earlier after being framed for plotting a coup of the Liones Kingdom (リオネス王国, Rionesu Ōkoku), the Holy Knights who sequestered them before taking control in the wake of a rebellion they organized.",
    otherNames: "seven deadly sins",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/TheDreamboy.jpg",
    animeName: "The Dream boy ",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "It tells the story of Naruto Uzumaki, a young ninja who seeks recognition from his peers and dreams of becoming the Hokage, the leader of his village. The story is told in two parts—the first set in Naruto's pre-teen years, and the second in his teens.",
    otherNames: "Naruto",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/TONIKAWA.jpg",
    animeName: "Tonikawa",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Gold Roger, the King of the Pirates, attained this and everything else the world had to offer. But 2 decades after his death, a young man named Luffy embarks on a journey across the world to discover it's greatest treasure, the One Piece, and inherit Roger's will - by becoming the King of the Pirates.",
    otherNames: "onepiece",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
];

export var dataDictionarydub = [
  {
    imageSrc: "../img/cards/TheDreamboy.jpg",
    animeName: "The Dream boy ",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "It tells the story of Naruto Uzumaki, a young ninja who seeks recognition from his peers and dreams of becoming the Hokage, the leader of his village. The story is told in two parts—the first set in Naruto's pre-teen years, and the second in his teens.",
    otherNames: "Naruto",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/TONIKAWA.jpg",
    animeName: "Tonikawa",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Gold Roger, the King of the Pirates, attained this and everything else the world had to offer. But 2 decades after his death, a young man named Luffy embarks on a journey across the world to discover it's greatest treasure, the One Piece, and inherit Roger's will - by becoming the King of the Pirates.",
    otherNames: "onepiece",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/Parasyte.jpg",
    animeName: "PARASYTE",
    starRate: "6.7",
    episodeCount: "6",
    releaseDate: "jul 14,2023",
    description:
      "The 16-year-old Izumi Shinichi lives with his mother and father in a quiet neighborhood in Tokyo. One night, worm-like aliens called Parasytes invade Earth, taking over the brains of human hosts by entering through their ears or noses.",
    otherNames: "PARASYTE",
    airedDate: "Oct 10, 2010",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/attackontitan.jpg",
    animeName: "Attack On Titan",
    starRate: "6.7",
    episodeCount: "6",
    releaseDate: "jul 14,2023",
    description:
      "It is set in a world where humanity is forced to live in cities surrounded by three enormous walls that protect them from gigantic man-eating humanoids referred to as Titans; the story follows Eren Yeager, who vows to exterminate the Titans after they bring about the destruction of his hometown and the death of his ...",
    otherNames: "Attack On Titan",
    airedDate: "Oct 10, 2010",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/bleach.jpg",
    animeName: "Bleach-season final",
    starRate: "9.7",
    episodeCount: "3",
    releaseDate: "jul 30,2023",
    description:
      "Ichigo Kurosaki is a teenager from Karakura Town who can see ghosts, a talent allowing him to meet a supernatural human Rukia Kuchiki, who enters the town in search of a Hollow, a kind of monstrous lost soul who can harm both ghosts and humans.",
    otherNames: "Bleach-season final",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/Free.jpg",
    animeName: "FREE!",
    starRate: "6.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Free! revolves around members of a high school boys' swimming team. Haruka Nanase, Makoto Tachibana, Rin Matsuoka and Nagisa Hazuki are members of the same swimming club in elementary school before parting ways, when Rin moves to Australia for schooling and specialized training in swimming.",
    otherNames: "FREE!",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/deathnote.jpg",
    animeName: "Death note",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "A high school student named Light Turner discovers a mysterious notebook that has the power to kill anyone whose name is written within its pages, and launches a secret crusade to rid the world of criminals.",
    otherNames: "Death note",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/sweet-reincarnation.jpg",
    animeName: "Sweet-reincarnation",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Nine-year-old Pastry Mille Morteln is from reincarnation and encounters the magical world. He must learn how to fight, utilizing his confectionary knowledge about pastry and discovering magic along the way.",
    otherNames: "Sweet-reincarnation",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/classrooma.jpg",
    animeName: "classroom asassian",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description: "",
    otherNames: "classroom asassian",
    airedDate: "Oct 20, 2016",
    status: "now",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/fairytail.jpg",
    animeName: "Fairy tail ",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      ". The world of Earth-land is home to numerous guilds where wizards apply their magic for paid job requests. Natsu Dragneel, a Dragon Slayer wizard from the Fairy Tail guild, explores the Kingdom of Fiore in search of his missing adoptive father, the dragon Igneel",
    otherNames: "fairy tail",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/demonslayer.jpg",
    animeName: "Demons slayer",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "Tanjiro sets out to become a demon slayer to avenge his family and cure his sister.A family is attacked by demons and only two members survive - Tanjiro and his sister Nezuko, who is turning into a demon slowly. Tanjiro sets out to become a demon slayer to avenge his family and cure his sister.",
    otherNames: "Demons slayer",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
  {
    imageSrc: "../img/cards/sevendeadlysins.jpg",
    animeName: "seven deadly sins",
    starRate: "8.7",
    episodeCount: "3",
    releaseDate: "jul 16,2023",
    description:
      "The Seven Deadly Sins are a band of knights in the land of Britannia (ブリタニア, Buritania) who had disbanded ten years earlier after being framed for plotting a coup of the Liones Kingdom (リオネス王国, Rionesu Ōkoku), the Holy Knights who sequestered them before taking control in the wake of a rebellion they organized.",
    otherNames: "seven deadly sins",
    airedDate: "Oct 20, 2016",
    status: "Releasing",
    genres: "Action, Adventure, Comedy, Drama, Fantasy, Shounen, Super Power",
  },
];

export var subanime = [
  {
    imageSrc: "../img/jjk.webp",
    animeName: "Jujutsu Kaisen",
    episodeNumber: "5",
  },
  {
    imageSrc: "../img/myhero.jpeg",
    animeName: "My Hero Academia",
    episodeNumber: "5",
  },
  {
    imageSrc: "../img/titan.webp",
    animeName: "Attack On Titan",
    episodeNumber: "5",
  },
  {
    imageSrc: "../img/tokyog.jpg",
    animeName: "Tokyo Ghoul",
    episodeNumber: "5",
  },

  {
    imageSrc: "../img/onepiece.jpg",
    animeName: "One Piece",
    episodeNumber: "5",
  },
  {
    imageSrc: "../img/mob.jpg",
    animeName: "Mob pycho",
    episodeNumber: "5",
  },
  {
    imageSrc: "../img/onepunch.webp",
    animeName: "one punch",
    episodeNumber: "5",
  },
  {
    imageSrc: "../img/bleach.webp",
    animeName: "Bleach",
    episodeNumber: "5",
  },
];

export var cardsmain = [
  {
    imgSrc: "../img/jjk.webp",
    animeName: "Jujutsu Kaisen",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/myhero.jpg",
    animeName: "Bleach",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/titan.webp",
    animeName: "Attack On Titan",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/bleach.webp",
    animeName: "Bleach",
    episodeNumber: "3",
  },

  {
    imgSrc: "../img/mob.jpg",
    animeName: "mob pycho",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/tokyog.jpg",
    animeName: "Bleach",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/onepunch.jpg",
    animeName: "One punch man",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/free.webp",
    animeName: "free!",
    episodeNumber: "3",
  },

  // second section
  {
    imgSrc: "../img/naruto.jpeg",
    animeName: "naruto",
    episodeNumber: "4",
  },
  {
    imgSrc: "../img/re-zero-season.png",
    animeName: "Re:zero",
    episodeNumber: "5",
  },
  {
    imgSrc: "../img/steins-gate.webp",
    animeName: "Steins-gate",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/tokyorevenger.jpeg",
    animeName: "tokyorevengers",
    episodeNumber: "3",
  },

  {
    imgSrc: "../img/chainsaw.webp",
    animeName: "chainsawman",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/beyblademetal.jpeg",
    animeName: "Beyblade",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/Seven-Deadly-Sins.webp",
    animeName: "Seven Deadly Sins",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/Toradora.webp",
    animeName: "Toradora!",
    episodeNumber: "3",
  },
  // third section
  {
    imgSrc: "../img/onepunch.jpg",
    animeName: "One punch man",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/free.webp",
    animeName: "free!",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/steins-gate.webp",
    animeName: "Steins-gate",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/tokyorevenger.jpeg",
    animeName: "tokyorevengers",
    episodeNumber: "3",
  },

  {
    imgSrc: "../img/chainsaw.webp",
    animeName: "chainsawman",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/beyblademetal.jpeg",
    animeName: "Beyblade",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/titan.webp",
    animeName: "Attack On Titan",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/bleach.webp",
    animeName: "Bleach",
    episodeNumber: "3",
  },

  // fourth section
  {
    imgSrc: "../img/deathnote.jpeg",
    animeName: "Deathnote",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/demonslayer.webp",
    animeName: "Demonslayer",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/TONIKAWA.jpg",
    animeName: "TONIKAWA",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/fairytail.webp",
    animeName: "fairytail",
    episodeNumber: "3",
  },

  {
    imgSrc: "../img/Bottom-TierCharacterTomozaki.webp",
    animeName: "Bottom-TierCharacterTomozaki",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/re-zero-season.png",
    animeName: "Re:zero",
    episodeNumber: "5",
  },
  {
    imgSrc: "../img/steins-gate.webp",
    animeName: "Steins-gate",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/tokyorevenger.jpeg",
    animeName: "tokyorevengers",
    episodeNumber: "3",
  },

  //fifth seciton
  {
    imgSrc: "../img/onepunch.jpg",
    animeName: "One punch man",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/free.webp",
    animeName: "free!",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/steins-gate.webp",
    animeName: "Steins-gate",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/tokyorevenger.jpeg",
    animeName: "tokyorevengers",
    episodeNumber: "3",
  },

  {
    imgSrc: "../img/chainsaw.webp",
    animeName: "chainsawman",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/beyblademetal.jpeg",
    animeName: "Beyblade",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/titan.webp",
    animeName: "Attack On Titan",
    episodeNumber: "3",
  },
  {
    imgSrc: "../img/bleach.webp",
    animeName: "Bleach",
    episodeNumber: "3",
  },

  // fifth section
];

export const photoUrls = [
  "../img/jjk.webp",
  "../img/myhero.jpg",
  "../img/titan.webp",
  "../img/tokyog2.png",
  "../img/ichigo_kurosaki.jpg",
  "../img/onepiece.jpg",
  "../img/onepunch.jpg",
  "../img/mob.jpg",
  // Add more photo URLs as needed
];
